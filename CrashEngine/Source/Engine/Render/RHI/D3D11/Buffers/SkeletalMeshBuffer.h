...
public:
    ...
    void Create(ID3D11Device* InDevice, const TMeshData<VertexType>& InMeshData)
    {
        ...
        VertexBuffer.Create(InDevice, InMeshData.Vertices.data(), VertexCount, sizeof(VertexType));
	...
	void                  UpdateVertex(ID3D11DeviceContext* Context, const void* Data, uint32 Count);
    void                  Release();
